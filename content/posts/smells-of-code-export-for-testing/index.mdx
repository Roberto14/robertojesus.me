...
There is also no silver bullet or a single **pattern** or way to fix this problem.
Each use case is unique and has its owns business requirements and complexity. Nevertheless,
the aim for refactoring should be focused on Single Responsibility Principle (SRT) and Dependency Injection (DI).
These patterns are known to improve the code testability and quality. //todo add reference
//todo: add reference
...
Dependency Injection (DI). With DI, our code dependencies are really easy to test as you can inject spies directly into
the implementation.
...
and it is safe to be exported.

##### 4. Does a private function really belongs to this module?
If it is safe to be exported, perhaps it could belong to other module or even become a
...
In Javascript ecosystem, we can use [rewire](https://www.npmjs.com/package/rewire) package to inspect private
...
In this naive example, we don't want to export `withSeparator` as it's a module private member.
The way we will test it is just by calling `appendElement` with separator flag set as true.
...
(see Deno *_internals* or *rewire* bellow), in my humble opinion, it brings more risks than benefits specially for
big codebases where the complexity is considerably higher.
...
3. [Rewire](https://www.npmjs.com/package/rewire)
